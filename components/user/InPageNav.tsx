...
export const InPageNav = ({
  ...
  return (
    <div className="bg-white rounded-md w-full border-[1px] border-gray-200 transition-all">
      {paths.map(({ route, name }, index) => {
        ...
        return (
          <Link
            href={route}
            key={route}
            className={`${activeCls} ${border} w-full block p-[22px] text-[#6A6B6C] hover:bg-primary/10`}
          >
            {name}
          </Link>
        )
